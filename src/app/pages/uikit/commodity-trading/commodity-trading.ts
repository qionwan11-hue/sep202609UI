...
@Component({
  selector: 'app-commodity-trading',
...
export class CommodityTrading {
  @Input() symbol: string = 'TVC:GOLD'; // Default to Bitcoin
  ...
  private updateBalanceViaAPI(): void {
    ...
    this.dashboardData.updateCommodityTrade(payload).subscribe({
      next: (res) => {
